...
│  ├─ curriculum.js           課程一覧
│  ├─ drill.js                大量ドリル（単語 → 母音を即答、弱点優先出題）
│  └─ util.js
├─ data/
│  ├─ curriculum.json         カリキュラム（編集可）
│  ├─ words.json              単語・発音記号・注記・minimal pairs（編集可）
│  ├─ drill-words.json        大量ドリル用の単語バンク（約1,550語。tests/build-drill-words.py で生成）
...
下部ナビの「ドリル」から。混同しやすい母音の組み合わせごとに大量の単語（/ɑ æ ʌ/ 407 語、/iː ɪ e/ 337 語など計約 1,550 語）を用意し、単語を見て母音を即答します。

- 出題数 20 / 50 / 100 / 無制限。数字キー 1〜3 でも回答可
- 「正解→自動で次へ」の高速モードと、「発音して『言えた』で次へ」の反復モード（反復カウントに加算）
- 未出題の語と過去に間違えた語を重み付きで優先し、各音から均等に出題
- 間違えた語は同じ回の 4〜7 問後に再出題、翌日以降も優先。分類ゲームの苦手語にも登録
- 単語ごとの正誤履歴（`drillWords`）とセッション結果（`drillLog`）を保存。ホームに「綴り→音の混同」を表示
- 「苦手だけ」で誤答率の高い語だけを回せる

単語は `data/drill-words.json` を直接編集するか、`tests/build-drill-words.py` のリストを編集して再生成します（同じ語が複数の音に登録されていないかをスクリプトが検査）。cot–caught merger の対象語（CLOTH 語: dog, off, cost など）は /ɔː/ に置き、注記を付けています。
...
| settings | 設定 |
| drillLog | 大量ドリル 1 回の結果（セット・出題数・正解数・混同・音別成績・平均秒） |
| drillWords | ドリル単語ごとの出題数・正解数・誤答数・最終誤答日 |
...
| `js/drill.js` `RETRY_GAP` | ドリルで誤答語を再出題する間隔 | 4〜7 問後 |
| `data/drill-words.json` `sets` | ドリルの音の組み合わせ | 10 セット |
